...
test("applied migrations live in the globbed directory and auth/ stays a template copy", () => {
  const migrationsDir = join(projectRoot(), "migrations");
  const pending = pendingMigrations(readdirSync(migrationsDir), []);
  assert.deepEqual(
    pending.map((row) => row.name),
    ["0001_auth.sql", "0002_app.sql", "0003_task_modes.sql", "0004_project_files.sql", "0005_evidence_ledger.sql", "0006_closed_loop.sql", "0007_provider_budget.sql", "0008_dynamic_council.sql", "0009_provider_scan.sql", "0010_nanogpt_billing.sql", "0011_council_member_identity.sql", "0012_durable_council_runs.sql", "0013_council_run_waker.sql", "0014_provider_credentials.sql", "0015_i18n.sql", "0016_repository_index.sql", "0017_evidence_snapshot.sql", "0018_solo_mode.sql"],
  );
