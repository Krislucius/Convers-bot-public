...
Recorded: 2026-09-04T09:20:00.000Z
...
OpenRouter (`sk-or-…`) and OpenRusRouter (`orr_live_…`). Keys persist on `account_settings` as soon as Save runs, even if the connection probe fails. Models and `max_cost_usd` persist with the account.
...
- Production Publish is a user action; git `CB-BUILD-20260904-023` is not `PROD_SYNC` until that host serves this `BUILD_ID`.
