...
Revision: CB-ARCH-20260906-002
Recorded: 2026-09-06T21:20:00.000Z
...
Selectable API providers: NanoGPT (`sk-nano-…`) and OpenRouter (`sk-or-…`). OpenRusRouter remains available for existing `orr_live_…` keys. NanoGPT has an explicit billing mode: Subscription (default) or Pay-as-you-go. Subscription Test Connection uses `GET /api/subscription/v1/models?detailed=true` as the only Council-eligible catalog. PAYG uses generic `/api/v1/models` only when the user selects Pay-as-you-go. Settings Test normalizes the catalog (direct array or OpenAI `{ data: [...] }`) and probes whether this account can actually call recommended/selected models on that billing API. Unsupported catalog shapes fail with `CATALOG_PARSE_ERROR`. Catalog presence is not treated as usable access. CONNECTED requires an authenticated provider check. Scan statuses: AVAILABLE / UNAVAILABLE / NOT_INCLUDED / UNKNOWN. Recommendations and Council membership use only AVAILABLE models from the current billing catalog. PAYG-only models never enter a Subscription Council. Before a paid Council run every selected model is pinged with a minimal authenticated completion on the frozen billing endpoint; Council starts only when each selected id is VERIFIED_AVAILABLE. A catalog-visible subscription-denied model is NOT_INCLUDED and blocks the run. Provider brand names are not models. Grok is not injected when absent. Stale selected IDs are dropped on provider, billing-mode, or catalog change. A failed Test Connection never shows a previous scan as current; the old catalog is STALE cached data. Test Connection always writes a sanitized persisted log (PASS and FAIL), including catalog HTTP status, response shape, parse metadata, and billing URLs. After save the browser does not keep the secret (`creds.apiKey` is empty); Council Run resolves the selected provider's key server-side. `assertRunCredentials` must not treat that empty client field as "not connected" when the account already shows READY for the selected provider. Settings Test Connection / Refresh models and Save share one canonical discovery apply (`settings-scan`). Save persists provider, billing, and API key, invalidates the current attempt, then runs that scan. Status, model counts, selected Council, error, and Test Log share one `attempt_id`. Save after a PASS cannot restore a previous FAILED scan. A leftover OpenRouter `sk-or-…` secret is not treated as a connected NanoGPT key, and a leftover NanoGPT secret is not treated as an OpenRouter key. Switching provider or NanoGPT billing re-runs discovery and never mixes providers or NanoGPT billing APIs inside one run.

## CURRENT COUNCIL FLOW

`src/routes/t.$taskId.tsx` → `runCouncil` in `src/lib/council/orchestrate.ts` only. CREATE / REVIEW / DECIDE. Dynamic 2–5 selected models. Round 1 independent, Round 2 cross-review, structured synthesis from a selected surviving model. Two surviving agents are enough. Fewer than two survivors is a PARTIAL RESULT: FAILED, successful responses stay visible, synthesis is skipped with an explicit reason, Retry failed models / Replace failed models / Restart Council. Each model is one agent card (name, status, attempts n/m, last error = exact sanitized failure). CREATE APPROVED writes an Implementation Packet. REVIEW returns PASS / PATCH / BLOCKED. DECIDE disagreements or CONFLICTED evidence require USER_DECISION_REQUIRED. Progress is persisted on `tasks.diagnostics.run` (stage, timestamps, per-agent state, `run_id`, provider, NanoGPT billing mode, members, request budget, cost telemetry, partial, synthesisSkipped). Stop marks `CANCELLED` and keeps partial agent rows. Restart issues a new `run_id` and drops stale writes from the previous generation. Catalog listing is not a start gate: a live selected-model verify blocks `MODEL_UNAVAILABLE` before paid `completeChat`. Empty completions are failures. Runtime failures keep provider, model, stage, HTTP status/class, named error code (`PAYG_BALANCE_REQUIRED`, `SUBSCRIPTION_LIMIT_REACHED`, `MODEL_NOT_INCLUDED`, `MODEL_UNAVAILABLE`, `RATE_LIMITED`, `PROVIDER_ERROR`), attempt, and retry exhaustion — never "provider error" and never “subscription credits exhausted” for PAYG 402. USD cost is telemetry; the hard gate is request attempts (for N=3: 7 expected / 12 ceiling). One selected provider, one NanoGPT billing mode, and the selected models are frozen on the task for that run. Subscription Council calls never use generic `/api/v1`.
...
- Production Publish is a user action; git `CB-BUILD-20260906-002` is not `PROD_SYNC` until that host serves this `BUILD_ID`.
