...
Positions, disagreements, blockers, resolved/unresolved issues, and citations are preserved on the Council result. The safety gate evaluates only the final unresolved issue set after Round 2 remaining findings and synthesis `resolved_issues` / `unresolved_issues` / `REJECTED_OBJECTIONS`. Each issue has a stable `issue_id` and exactly one disposition (`UNRESOLVED`, `RESOLVED`, `REJECTED`, `ACCEPTED_AS_PATCH`, `USER_DECISION_REQUIRED`). Duplicate reviewer P0s collapse to one id. Unresolved P0 (and REVIEW/DECIDE P1) force BLOCKED even if the synthesizer proposed APPROVED. Resolved or rejected P0 does not block. CREATE `P1_ARCHITECTURE` reconstruction notes are not unresolved defects. Empty / none / n/a / dash lists are not issues. Persisted fields include `proposed_status`, `reconciled_status`, the issue ledger, and `gate_reason`. Task status COMPLETE is execution only and is distinct from the semantic verdict. COMPLETE, FAILED, and CANCELLED are mutually exclusive: a reconcilable synthesis already persisted cannot be written as CANCELLED, and the UI shows one terminal banner. An Implementation Packet is written only for CREATE when the reconciled verdict is APPROVED and the run is COMPLETE.
