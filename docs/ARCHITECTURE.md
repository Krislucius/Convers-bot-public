...
The browser never owns execution. Closing the tab, reload, or logout does not cancel a run. UI poll is reconnect-only. After a completed run the task page shows one Decision Record first: run status (COMPLETE / FAILED / CANCELLED) separate from task verdict (APPROVED / PATCH / BLOCKED / USER_DECISION_REQUIRED), agreed points, open P0 blockers, and the next action. Raw synthesis, model positions, evidence ledger, and technical metadata stay collapsed.
