...
describe("systemIdentity", () => {
  it("exposes sourceCommit without changing the rest of the identity", () => {
    ...
    assert.equal(id.buildId, "CB-BUILD-20260908-001");
    assert.equal(id.architectureRevision, "CB-ARCH-20260908-001");
  });
  ...
  it("treats SOURCE_COMMIT=UNKNOWN as diagnostic, not a health failure", () => {
    ...
    assert.equal(productionSync("CB-BUILD-20260908-001"), true);
  });
