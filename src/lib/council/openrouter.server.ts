...
export function operatorError(err: unknown, apiKey = ""): string {
  ...
  if (
    ...
    low.includes("did not recognize")
  ) {
    return keyRejectedMessage(401, raw, keyFingerprint(apiKey));
  }
  if (low.includes("not currently available") || low.includes("unpublished") || low.includes("unknown model")) {
    return raw;
  }
  if (low.includes("not connected") || low.includes("api key")) {
...
export async function complete(opts: {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    if (res.status === 429 || res.status >= 500) {
      last = `OpenRouter HTTP ${res.status}`;
